class SessionRatingsController < ApplicationController
  ...
  def index
    @session_ratings = SessionRating.all.includes(:user, enrollment: [:training_session]).page(params[:page])
  end
  ...
  def create
    ...
    set_user_type @session_rating
    respond_to do |format|
      if @session_rating.save
        format.html { redirect_to @session_rating.enrollment.training_session, notice: 'Session rating was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @session_rating.update(session_rating_params)
        format.html { redirect_to @session_rating.enrollment.training_session, notice: 'Session rating was successfully updated.' }
  ...
  def set_user_type session_rating
    session_rating.enrollment.nil? ? (session_rating.user_type = "INSTRUCTOR") : (session_rating.user_type = "STUDENT")
  end
  ...
  def session_rating_params
    params.require(:session_rating).permit(:rating, :user_type, :comment, :enrollment_id, :user_id, session_rating_reason_ids:[])
  end
end
